...
SELECT * FROM public_data_science.betting_receipts
...
SELECT user_id,
       max(all_transactions.amount) max_betting_win_amount,
	   min(all_transactions.amount) min_betting_win_amount,
	   sum(all_transactions.amount) total_betting_win_amount,
	   count(transaction_code) num_betting_wins,
 	   agg_trans.avg_daily_trans avg_daily_betting_w_transactions,
	   agg_trans.avg_daily_amount avg_daily_betting_w_amount,
	   agg_trans.avg_weekly_trans avg_weekly_betting_w_transactions,
	   agg_trans.avg_weekly_amount avg_weekly_betting_w_amount,
	   agg_trans.avg_monthly_trans avg_monthly_betting_w_transactions,
	   agg_trans.avg_monthly_amount avg_monthly_betting_w_amount
FROM all_transactions
